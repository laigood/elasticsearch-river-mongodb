...
public class MongoDBRiver extends AbstractRiverComponent implements River {
  ...
  protected boolean authOplog = false;
  protected boolean authDb = false;
  ...
  protected BasicDBObject oplogRetrieveFields;
  protected BasicDBObject slurpRetrieveFields;
  ...
  public MongoDBRiver(final RiverName riverName, final RiverSettings settings,
    ...
    if (settings.settings().containsKey(RIVER_TYPE)) {
      ...
      if (mongoSettings.containsKey(USER_FIELD) && mongoSettings.containsKey(PASSWORD_FIELD)) {
        dbUser     = mongoSettings.get(USER_FIELD).toString();
        dbPassword = mongoSettings.get(PASSWORD_FIELD).toString();
      } else {
        dbUser = "";
        dbPassword = "";
      }
      ...
      if (mongoSettings.containsKey("fields")) {
        String fields = (String) mongoSettings.get("fields");
        initFields(fields);
      }
    } else {
      mongoHost = "localhost";
  ...
  private void initFields(String str) {
    String[] fields;
    if(null == str || (fields = str.split(",")).length == 0){
      return;
    }
    slurpRetrieveFields = new BasicDBObject();
    oplogRetrieveFields = new BasicDBObject();
    for (String field : fields) {
      field = field.trim();
      slurpRetrieveFields.put(field, 1);
      oplogRetrieveFields.put("o." + field, 1);
    }
    oplogRetrieveFields.append("ts", 1)
                       .append("h",  1)
                       .append("op", 1)
                       .append("ns", 1)
                       .append("o2", 1)
                       .append("o._id", 1);
    logger.info("fields:[{}]", slurpRetrieveFields.keySet());
  }
  ...
  public void start() {
    ...
    tailerThread  = EsExecutors.daemonThreadFactory(settings.globalSettings(), "mongodb_river_slurper").newThread(new Slurper());
  ...
  private class Slurper implements Runnable {
    ...
    private boolean asignCollections() {
      ...
      if (!dbUser.isEmpty() && !dbPassword.isEmpty()&&!authDb) {
        boolean auth = slurpedDb.authenticate(dbUser, dbPassword.toCharArray());
        if (auth == false) {
          logger.warn("Invalid credential");
          return false;
        }
    ...
    private DBCursor oplogCursor(final BSONTimestamp timestampOverride) {
      ...
      return oplogCollection.find(indexFilter, oplogRetrieveFields) //TODO fields
    ...
    private void addQueryToStream(final String operation, final BSONTimestamp currentTimestamp, final DBObject update) {
      for (DBObject item : slurpedCollection.find(update, slurpRetrieveFields)) { //TODO fields
